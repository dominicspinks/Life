...
import { ionPencil, ionTrashBin, ionAdd } from '@ng-icons/ionicons';
...
export class BudgetSummaryTabComponent {
    ...
    @ViewChild('cashFlowDescriptionInput')
    cashFlowDescriptionInput!: ElementRef<HTMLInputElement>;

    showDeleteModal = false;
    deleteCashflow: BudgetCashFlow | null = null;
    ...
    getTotal(week: number, categoryId: number): number | null {
        return (
            this.weeklySummary.find(
                (w) => w.week === week && w.category === categoryId
            )?.total ?? null
        );
    }
    ...
    getWeekTotal(week: number): number {
        return this.weeklySummary
            ...
            .reduce((sum, row) => {
                if (
                    this.budgetConfiguration?.categories.find(
                        (c) => c.id === row.category
    ...
    loadSummaryData(): void {
        ...
        this.budgetService
            .getPurchaseSummary(this.budgetId, start, end)
            .subscribe({
                next: (res) => {
    ...
    get summaryRows(): { name: string; values: Record<number, number> }[] {
        ...
        for (const entry of this.weeklySummary) {
            if (!dataByCategory[entry.category])
                dataByCategory[entry.category] = [];
            dataByCategory[entry.category].push(entry.total);
        }

        const latestWeek = Math.max(
            ...this.weeklySummary.map((w) => w.week),
            1
        );
        ...
        const includedCategories = categories.filter(
            (c) => !c.excluded_from_budget
        );

        totals[0] = includedCategories.reduce(
            (sum, c) => sum + (totals[c.id!] || 0),
            0
        );
        averages[0] = includedCategories.reduce(
            (sum, c) => sum + (averages[c.id!] || 0),
            0
        );
        targets[0] = includedCategories.reduce(
            (sum, c) => sum + (targets[c.id!] || 0),
            0
        );
        variances[0] = includedCategories.reduce(
            (sum, c) => sum + (variances[c.id!] || 0),
            0
        );
    ...
    getCellColour(week: number, categoryId: number): string {
        ...
        if (categoryId === 0) {
            const included = this.orderedCategories.filter(
                (c) => !c.excluded_from_budget
            );
            value = this.getWeekTotal(week);
            target = included.reduce((sum, c) => sum + c.weekly_target, 0);
        } else {
            const category = this.orderedCategories.find(
                (c) => c.id === categoryId
            );
            if (!category) return '';
    ...
    getAverageCellColour(categoryId: number): string {
        if (categoryId === 0) {
            ...
            const target = included.reduce(
                (sum, c) => sum + c.weekly_target,
                0
            );
            const average =
                this.summaryRows.find((r) => r.name === 'Average')?.values[0] ??
                0;
        ...
        const average =
            this.summaryRows.find((r) => r.name === 'Average')?.values[
                categoryId
            ] ?? 0;

        if (average <= category.weekly_target) return this.statusColours.ok;
        if (average < 1.2 * category.weekly_target)
            return this.statusColours.warning;
        return this.statusColours.bad;
    }
    ...
    getVarianceCellColour(categoryId: number): string {
        const variance =
            this.summaryRows.find((r) => r.name === 'Variance')?.values[
                categoryId
            ] ?? 0;
        const target =
            this.summaryRows.find((r) => r.name === 'Weekly Target')?.values[
                categoryId
            ] ?? 0;

        if (variance <= 0) return this.statusColours.ok;
        if (variance < 0.2 * target * this.activeWeek)
            return this.statusColours.warning;
    ...
    updateForecastProjection(): void {
        ...
        const weeklyAvg =
            this.summaryRows.find((r) => r.name === 'Average')?.values[0] ?? 0;
        const yearlyAvg = weeklyAvg * 52;

        const weeklyTarget =
            this.summaryRows.find((r) => r.name === 'Weekly Target')
                ?.values[0] ?? 0;
        ...
        for (const cf of this.forecastCashFlows) {
            const cfMultiplier = this.getPeriodMultiplier(
                cf.period_name as PeriodName
            );
    ...
    confirmDeleteCashFlow(cashflow: BudgetCashFlow): void {
        ...
        this.deleteCashflow = cashflow;
    }

    closeDeleteModal(): void {
        this.showDeleteModal = false;
        this.deleteCashflow = null;
    }

    deleteCashFlow(): void {
        if (!this.deleteCashflow?.id) return;
        this.budgetService
            .deleteCashFlow(this.budgetId, this.deleteCashflow.id)
            .subscribe({
                next: () => {
                    this.forecastCashFlows = this.forecastCashFlows.filter(
                        (c) => c.id !== this.deleteCashflow?.id
                    );
    ...
    saveCashFlow(): void {
        ...
        const req$ = payload.id
            ? this.budgetService.updateCashFlow(
                  this.budgetId,
                  payload.id,
                  payload
              )
        ...
        req$.subscribe({
            next: (saved: BudgetCashFlow) => {
                const index = this.forecastCashFlows.findIndex(
                    (c) => c.id === saved.id
                );
                if (index >= 0) {
    ...
    sortCashFlows(): void {
        this.forecastCashFlows.sort((a, b) => {
            return a.is_income && b.is_income
                ? a.id! - b.id!
                : a.is_income
                ? -1
                : 1;
